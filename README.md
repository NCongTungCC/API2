...
Start MySQL before starting the API. The default local settings expect MySQL at
`localhost:3306`, with database `typescript`, user `root`, and password `root123`.

Alternatively, start the complete development stack with Docker Desktop running:

```bash
docker compose up
```

The API will be available at `http://localhost:5000` and the health check at
